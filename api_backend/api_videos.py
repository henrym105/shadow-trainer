...
from fastapi import Query, UploadFile, File, HTTPException, APIRouter
...
# FastAPI router
# ----------------------------------------------------
# These endpoints will all have a "/videos" prefix 
sub_app_videos = APIRouter(
    tags=["Videos"]
)
...
@sub_app_videos.get("/")
def read_root():
    """Root endpoint: returns welcome message and number of tasks in OUTPUT_DIR."""
    try:
        # Count directories ending with '_output' in OUTPUT_DIR
        task_dirs = [d for d in OUTPUT_DIR.iterdir() if d.is_dir() and d.name.endswith('_output')]
        num_tasks = len(task_dirs)
    except Exception as e:
        logger.error(f"Error counting tasks in OUTPUT_DIR: {e}")
        num_tasks = None
    return {
...
@sub_app_videos.post("/sample-lefty")
...
@sub_app_videos.post("/get_3d_keypoints")
...
@sub_app_videos.post("/{task_id}/terminate")
...
@sub_app_videos.post("/{task_id}/generate-evaluation")
...
@sub_app_videos.get("/{task_id}/status")
...
@sub_app_videos.get("/{task_id}/download")
...
@sub_app_videos.get("/{task_id}/preview/original")
...
@sub_app_videos.get("/{task_id}/preview/processed")
...
@sub_app_videos.get("/{task_id}/keypoints/user")
...
@sub_app_videos.get("/{task_id}/keypoints/pro")
