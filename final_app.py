...
# ---------------------------------------------------------
# Optional: Compare to pro pitcher (added for comparison feature)
# ---------------------------------------------------------
# Compare to professional pitcher (Shane Bieber added)
compare_pitcher = st.selectbox(
    "Would you like to compare yourself with a professional pitcher?",
    options=["None", "Shohei Ohtani", "Max Scherzer", "Justin Verlander", "Shane Bieber"],
    index=0
)

# Optional: Map pitcher to fixed S3 video (used by backend if needed)
pitcher_video_map = {
    "Shane Bieber": "https://shadow-trainer-dev.s3.us-east-2.amazonaws.com/cropped_videos/BieberShaneR/FF/cropped_03587bf2-48bb-45f5-84b5-38eec993695e_0.mp4"
}
...
if input_mode == "Local File":
    ...
    # Show previews of videos in the videos/ directory
...
if st.button("Process Video"):
    with st.spinner("Processing video, please wait..."):
        ...
        if input_mode == "Local File" and video_file is not None:
            ...
            # Add pitcher comparison if selected
            if compare_pitcher != "None":
                params["compare_pitcher"] = compare_pitcher
            ...
            if compare_pitcher != "None":
                params["compare_pitcher"] = compare_pitcher
            if compare_pitcher in pitcher_video_map:
                params["compare_pitcher_video"] = pitcher_video_map[compare_pitcher]
    # Optional: tell backend whether to force reprocess
            params["compare_force_reprocess"] = False  # set True only if needed
